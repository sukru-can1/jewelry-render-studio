...
def main() -> None:
    ...
    template = request_json(
        ...
        "/templates",
        {
            "name": f"jewelry-render-blender-v027-blender5-slim-template-{int(time.time())}",
    ...
    print(json.dumps({"templateId": template_id}, indent=2))

    request_json(
        "PATCH",
        f"/endpoints/{ENDPOINT_ID}",
        {"workersMin": 0, "workersMax": 0, "idleTimeout": 1, "scalerType": "QUEUE_DELAY", "scalerValue": 4},
    )
    time.sleep(20)
    endpoint = request_json(
        ...
        f"/endpoints/{ENDPOINT_ID}",
        {
            "name": "jewelry-render-blender-v027-blender5-slim",
    ...
    print(json.dumps({"endpointId": endpoint["id"], "templateId": endpoint["templateId"]}, indent=2))
